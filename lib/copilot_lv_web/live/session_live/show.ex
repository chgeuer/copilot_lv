defmodule CopilotLvWeb.SessionLive.Show do
  ...
  def handle_info({:session_event, event}, socket) do
    ...
    acc = socket.assigns.accumulator
    {acc, actions} = Accumulator.process_event(acc, type, data)
    ...
    case actions do
      ...
      actions when is_list(actions) ->
        socket =
          ...
          |> apply_accumulator_actions(actions)
          |> then(fn s ->
            # Track file tokens for assistant message chunks
            if type == "assistant.message" do
              chunk = data["chunkContent"] || data["content"] || ""
              push_file_tokens_for_text(s, chunk)
            else
              s
            end
          end)
  ...
  @doc false
  def models_for_agent(agent), do: CopilotLv.ModelCatalog.for_agent(agent)
